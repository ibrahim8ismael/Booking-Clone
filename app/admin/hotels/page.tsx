...
export default function AdminHotelsPage() {
  ...
  const t = {
    ...
    addHotel: isAr ? 'إضافة فندق' : 'Add Hotel',
    searchPlaceholder: isAr ? 'البحث عن فندق...' : 'Search hotels...',
    allHotels: isAr ? 'جميع الفنادق' : 'All Hotels',
    totalRooms: isAr ? 'إجمالي الغرف' : 'Total Rooms',
    activeHotels: isAr ? 'الفنادق النشطة' : 'Active Hotels',
    ...
    manageRooms: isAr ? 'إدارة الغرف' : 'Manage Rooms',
    viewDetails: isAr ? 'عرض التفاصيل' : 'View Details',
    edit: isAr ? 'تعديل' : 'Edit',
    delete: isAr ? 'حذف' : 'Delete',
    cancel: isAr ? 'إلغاء' : 'Cancel',
    deleteConfirmTitle: isAr ? 'تأكيد الحذف' : 'Confirm Deletion',
    deleteConfirmDesc: isAr ? 'هل أنت متأكد من أنك تريد حذف هذا الفندق وجميع غرفه؟ هذا الإجراء لا يمكن التراجع عنه.' : 'Are you sure you want to delete this hotel and all its rooms? This action cannot be undone.',
  ...
  const totalRoomsCount = hotels.reduce((acc, h) => acc + h.totalRooms, 0);
  const availableRoomsCount = hotels.reduce((acc, h) => acc + h.availableRooms, 0);

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-2xl font-bold text-gray-900">{t.hotels}</h1>
        <Button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-gray-500 font-medium">{t.allHotels}</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-gray-500 font-medium">{t.activeHotels}</p>
            <p className="text-2xl font-bold text-gray-900">{activeHotelsCount}</p>
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-gray-500 font-medium">{t.totalRooms}</p>
            <div className="flex items-baseline gap-2">
              <p className="text-2xl font-bold text-gray-900">{totalRoomsCount}</p>
              <p className="text-sm text-gray-500 font-medium">({availableRoomsCount} {t.available})</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left" dir={isAr ? 'rtl' : 'ltr'}>
            <thead className="bg-gray-50 text-gray-500 text-sm border-b border-gray-200 align-bottom">
              <tr>
                ...
                <th className="px-6 py-4 font-medium">
                  <div>{t.totalRooms}</div>
                  <div className="text-gray-900 font-bold mt-1 text-lg">{totalRoomsCount}</div>
                </th>
                <th className="px-6 py-4 font-medium">
                  <div>{t.available}</div>
                  <div className="text-green-600 font-bold mt-1 text-lg">{availableRoomsCount}</div>
                </th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredHotels.length > 0 ? filteredHotels.map((hotel) => (
                <tr key={hotel.id} className="hover:bg-gray-50 transition-colors text-sm">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 text-xs mt-0.5">{hotel.id}</p>
                      </div>
                    </div>
                  </td>
                ...
                <tr>
                   <td colSpan={7} className="px-6 py-12 text-center text-gray-500">
                      {isAr ? 'لم يتم العثور على فنادق.' : 'No hotels found.'}
                   </td>
                </tr>
              )}
            </tbody>
