...
export default function AdminEditTourPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [groups, setGroups] = useState([
    ...
    { id: '2', name: 'Group 2', startDate: '2026-05-06', endDate: '2026-05-10', price: 200, adultCap: 20, childCap: 10, count: 1 }
  ]);
  ...
  const t = {
    back: isAr ? 'العودة إلى الرحلة' : 'Back to Tour',
    editTour: isAr ? 'تعديل بيانات الرحلة' : 'Edit Tour Details',
    ...
    address: isAr ? 'نقطة التجمع' : 'Meeting Point',
    ...
    addGroup: isAr ? 'إضافة مجموعة' : 'Add Group',
    ...
    numberOfGroups: isAr ? 'عدد المجموعات (المخزون)' : 'Groups Count',
  ...
  const featuresList = [
    { id: 'wifi', label: isAr ? 'واي فاي مجاني ضمن الفندق' : 'Free WiFi in Hotel', checked: true },
    { id: 'parking', label: isAr ? 'موقف سيارات مجاني' : 'Free Parking', checked: false },
    { id: 'transportation', label: isAr ? 'انتقالات داخلية' : 'Internal Transportation', checked: true },
    { id: 'guide', label: isAr ? 'مرشد سياحي مرافق' : 'Tour Guide', checked: true },
    { id: 'meals', label: isAr ? 'وجبات (إفطار أو غيرها)' : 'Meals Included', checked: true },
    { id: 'insurance', label: isAr ? 'تأمين سفر' : 'Travel Insurance', checked: false },
  ];
    ...
    <div className="pb-10 max-w-6xl mx-auto" dir={isAr ? 'rtl' : 'ltr'}>
      <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t.editTour} <span className="text-gray-400 font-normal ml-2">#{id}</span></h1>
            <p className="text-gray-500 text-sm mt-1">{isAr ? 'تعديل بيانات الرحلة والمواعيد' : 'Update tour details and schedules'}</p>
          </div>
        </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="space-y-2 md:col-span-3">
                  ...
                  <Input defaultValue={tour.name} className="bg-gray-50 h-11 border-gray-200" />
                </div>
          ...
          <section className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
            ...
            <div className="space-y-3">
              <label className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors">
                <input type="radio" name="status" defaultChecked={tour.status === 'Active'} className="w-4 h-4 text-[#0071c2] focus:ring-[#0071c2]" />
                <span className="font-medium text-gray-900">{t.active}</span>
              </label>
              <label className="flex items-center gap-3 p-3 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors">
                <input type="radio" name="status" defaultChecked={tour.status === 'Inactive'} className="w-4 h-4 text-[#0071c2] focus:ring-[#0071c2]" />
           ...
           <section className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input defaultValue={tour.city} className="bg-gray-50 h-11 border-gray-200" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-bold text-gray-700">{t.country}</label>
                <Input defaultValue={tour.country} className="bg-gray-50 h-11 border-gray-200" />
              </div>
              <div className="space-y-2">
            ...
            <div className="space-y-3">
              {featuresList.map(feature => (
                <label key={feature.id} className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" defaultChecked={feature.checked} className="w-4 h-4 rounded border-gray-300 text-yellow-500 focus:ring-yellow-500" />
